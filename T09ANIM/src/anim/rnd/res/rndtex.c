...
/* Array of textures */
va6TEXTURES VA6_RndTextures[VA6_MAX_TEXTURES];
/* Textures array store size */
INT VA6_RndTexturesSize;
...
INT VA6_RndTexAddImg( CHAR *Name, INT W, INT H, INT C, VOID *Bits )
{
  ...
  if (VA6_RndTexturesSize >= VA6_MAX_TEXTURES)
    return -1;
 
  /* Setup OpenGL texture */
  glGenTextures(1, &VA6_RndTextures[VA6_RndTexturesSize].TexId);
  glBindTexture(GL_TEXTURE_2D, VA6_RndTextures[VA6_RndTexturesSize].TexId);
  ...
  return VA6_RndTexturesSize++;
} /* End of 'VA6_RndTexAddImg' function */
 ...
 *   (INT) texture stock number (-1 if error is occured).
 */
INT VA6_RndTexAddFromFile( CHAR *FileName )
{
  INT ret = -1;
  HBITMAP hBm;
  FILE *F;
  ...
  if ((hBm = LoadImage(NULL, FileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION)) != NULL)
  {
    ...
    if (Bm.bmBitsPixel == 24 || Bm.bmBitsPixel == 32 || Bm.bmBitsPixel == 8)
      ret = VA6_RndTexAddImg(FileName, Bm.bmWidth, Bm.bmHeight, Bm.bmBitsPixel >> 3, Bm.bmBits); 
    DeleteObject(hBm);
    return ret;
  }
  if ((F = fopen(FileName, "rb")) != NULL)
  {
    ...
    if (components != -1)
      if ((mem = malloc(w * h * components)) != NULL)
      {
        ...
        ret = VA6_RndTexAddImg(FileName, w, h, components, mem); 
        free(mem);
      }
    fclose(F);
  }
  return ret;
} /* End of 'VA6_RndTexAddFromFile' function */
